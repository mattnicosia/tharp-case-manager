...
REQS_INITIAL[14] = {
  ...
  notes: `REQ-15 — APPLICATION #15 (FINAL REQUISITION) — Period 12/01/23 to 12/30/23
...
KEY FINDINGS:
  ...
  10. 7-MONTH GAP: No requisitions from May through November 2023. Final REQ bills
...
function Dashboard({ reqs, claims }) {
  ...
  const totalRetainage = reqs.reduce((s, r) => s + (r.retainageHeld || 0), 0);
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 12 }}>
        ...
        <KPI label="Retainage Held" isMoney rawAmount={totalRetainage} sub="Withheld by owners" color={T.red} />
      ...
      <Card>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            ...
            { req: "REQ-15", variance: "$0.00", reason: "ZERO VARIANCE — ONLY requisition where G703 TP ($6,451.64) matches Excel exactly. FINAL requisition after 7-month gap. However: $135,385 in UNAPPLIED credit COs at 0% — PCO#130 'GC Owner Variance Split' ($66,389), PCO#129 'Allowance Reconciliation' ($24,248), plus 5 other credits never applied to owner. CO total DECREASED $90,637 — unprecedented. Only 2 backup pages; both invoices out-of-period (ABC Supply 19 months, DeLeonardis 5 months). No timecards.", severity: "amber" },
          ].map(item => (
...
const PARTY_ANALYSIS = {
  mc: {
    ...
    weaknesses: [
      "Systemic documentation failures across all 15 requisitions (§15.3.2)",
...
const DOCS_INITIAL = [
  ...
  { id: "doc-018", category: "requisition", name: "REQ-15 — November 2023", date: "2023-11-30", description: "Fifteenth and final requisition.", parties: "MC", status: "submitted", notes: "" },
...
function Documents({ docs, updateDoc, addDoc, removeDoc }) {
  ...
  const filtered = docs.filter(d => {
    ...
    if (search) {
      ...
      return d.name.toLowerCase().includes(q) || d.description.toLowerCase().includes(q) || (d.notes || "").toLowerCase().includes(q);
    }
    return true;
  });
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: editing || adding ? "1fr 400px" : "1fr", gap: 20, alignItems: "start" }}>
      <div>
        <SectionTitle title="Documents" subtitle={`${docs.length} documents across ${DOC_CATEGORIES.length} categories`} />

        {/* Category Chips */}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
          <button onClick={() => setFilter("all")} style={{
            padding: "6px 14px", borderRadius: 20, cursor: "pointer", fontFamily: T.font, fontSize: 12, fontWeight: 500,
            border: `1.5px solid ${filter === "all" ? T.accent : T.border}`,
            background: filter === "all" ? T.accentBg : T.surface,
            color: filter === "all" ? T.accent : T.textMid,
        ...
        <Card padding={0}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["", "Document Name", "Date", "Parties", "Status"].map(h => (
                  <th key={h} style={ThStyle}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(doc => {
                ...
                return (
                  <tr key={doc.id} onClick={() => { setSel(active ? null : doc.id); setAdding(false); }}
                    ...
                    onMouseEnter={e => { if (!active) e.currentTarget.style.background = T.surfaceHover; }}
                    onMouseLeave={e => { if (!active) e.currentTarget.style.background = "transparent"; }}>
                    <td style={{ ...TdStyle, width: 36, textAlign: "center", fontSize: 16 }}>{cat.icon}</td>
                    <td style={TdStyle}>
                      <div style={{ fontWeight: 500 }}>{doc.name}</div>
                      <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>{doc.description}</div>
                    </td>
                    <td style={{ ...TdStyle, fontFamily: T.mono, fontSize: 12, whiteSpace: "nowrap" }}>{doc.date}</td>
                    <td style={{ ...TdStyle, fontSize: 12 }}>{doc.parties}</td>
                    <td style={TdStyle}>
                      <span style={{
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} style={{ ...TdStyle, textAlign: "center", color: T.textMuted, padding: 40 }}>No documents match your filter</td></tr>
              )}
            </tbody>
          </table>
        </Card>
          ...
          <Card>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea value={editing.notes || ""} onChange={e => updateDoc(editing.id, { notes: e.target.value })}
                rows={4} style={{ width: "100%", padding: "8px 10px", borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 12, fontFamily: T.font, color: T.text, background: T.surface, resize: "vertical" }} />
            </div>
          </Card>
        </div>
      )}
